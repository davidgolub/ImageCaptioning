...
local ImageCaptioner_Hidden = torch.class('imagelstm.ImageCaptioner_Hidden')

function ImageCaptioner_Hidden:__init(config)
...
function ImageCaptioner_Hidden:add_modules(parallel_net, module_list)
...
function ImageCaptioner_Hidden:train(dataset)
  ...
  for i = 1, dataset.size, self.batch_size do
    ...
    local feval = function(x)
      ...
      for j = 1, batch_size do
        ...
        local inputs = self.combine_layer:forward(sentence)
        ...
        self.combine_layer:backward(sentence, input_grads)
      end
...
function ImageCaptioner_Hidden:predict(image_features, beam_size)
  if self.gpu_mode then
...
function ImageCaptioner_Hidden:predict_dataset(dataset, beam_size, num_predictions)
...
function ImageCaptioner_Hidden:save_predictions(predictions_save_path, loss, test_predictions)
...
function ImageCaptioner_Hidden:print_config()
...
function ImageCaptioner_Hidden:save(path)
  local config = {
...
function ImageCaptioner_Hidden:getPath(epoch) 
...
function ImageCaptioner_Hidden.load(path)
